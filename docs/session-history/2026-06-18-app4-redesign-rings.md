# Redisseny App4 "Metrónomo Fracción" — anells concèntrics (TANCAT 2026-06-18)

> **ARXIU.** Redisseny complet (F1–F8) acabat i committejat el 2026-06-18. Aquest
> fitxer era `SESSION_STATE.md`; es mou a l'historial en completar F8. Els logs de
> fase de sota es conserven com a referència; el detall fi viu a la història de git.
>
> **F8 (neteja + docs)** fet en aquesta darrera tanda:
> - Auditoria extensa (workflow de 34 agents, criteris de /nuzic-migrate Steps
>   4/12/13/14/15): 38 troballes consolidades, 24 accionables verificades
>   adversarialment (efectes laterals d'init, lliçó A-17), 4 refutades.
> - **Neteja de codi** (commit 0b306dd, −221 línies netes): App4/main.js −155
>   (setters d'àudio orfes + toggleControllers, currentFractionInfo write-only,
>   maquinària del botó-títol, bindUnit focus-reveal inert, updateFormula mort i
>   tota la cascada formulaRenderer, branca complexfractions inabastable);
>   App4/styles.css −55 (CSS botó-títol, .hover-tip.validation-warning,
>   --unit-label-offset no-op; spinner gap px→rem); App1 (destructure mort
>   baseSoundSelect/startSoundSelect, comentaris obsolets, --unit-label-offset);
>   circular-timeline-ring.js (positionCircularRingNumbers deixa de ser export).
> - **Docs** (commit 88fb8a2): README d'usuari App4 (reescrit) + App1 (nou),
>   AGENTS.md d'App4/App1 reescrits, docs/MODULES.md + libs/app-common/CLAUDE.md +
>   CLAUDE.md arrel + App1/CLAUDE.md actualitzats (comptes 79/1502, mòduls nous).
> - **BLOQUEJAT** (no tocat): `timeline-renderer.js` el consumeix App4B (congelat);
>   `@media(max-height:680px)` i `@media(max-width:600px)` són ajustos responsive
>   intencionals (refutats com a "legacy"). DEFERIT (decisió de l'usuari): dedup
>   transversal de menu-toggle/random-config entre App1/2/4/5 (alt blast-radius).
> - Verificat: 79 suites / 1502 tests + smoke CDP (App4/App1 carreguen sense
>   errors de consola; randN habilitat confirma el refactor de complexfractions).

## Tasca (ARXIVADA): App4 → nuzic + 3 fraccions + cercles concèntrics
...
- [x] **F8 — Neteja + docs** ✅ (2026-06-18, vegeu el resum del capçal):
      auditoria extensa verificada → neteja de codi mort (commit 0b306dd) + docs
      (commit 88fb8a2) + arxiu. timeline-renderer.js queda (bloquejat per App4B);
      dedup transversal DEFERIT.
