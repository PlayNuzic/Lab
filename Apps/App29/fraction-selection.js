// Fraction selection logic for App29 (Complejos)
// Simplified version without random config (App29 has fixed parameters)
...
export function createFractionSelectionFromValue(value, options = {}) {
  ...
  const numerator_step = Number.isFinite(options.numerator) && options.numerator > 0
    ? Math.round(options.numerator)
    : 1;
...
export function fractionDisplay(base, numerator, denominator, { cycleIndex, subdivisionIndex, pulsesPerCycle } = {}) {
  ...
  // For App29, use simple base.subdivision format
  return `${safeBaseValue}.${safeNumeratorValue}`;
}
