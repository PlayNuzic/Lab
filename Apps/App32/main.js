...
function handleRandom() {
  ...
  // Generate random notes on the grid
  const maxSubdivs = getTotalSubdivisions();
  const newNotes = [];

  // Generate 3-8 random notes
  const numNotes = Math.floor(Math.random() * 6) + 3;

  for (let i = 0; i < numNotes; i++) {
    const note = Math.floor(Math.random() * NOTE_COUNT);
    const startSubdiv = Math.floor(Math.random() * (maxSubdivs - 1));
    const maxDuration = maxSubdivs - startSubdiv;
    const duration = Math.floor(Math.random() * Math.min(maxDuration, newD * 2)) + 1;

    // Check for overlap on same note row
    const overlaps = newNotes.some(n =>
      n.note === note &&
    ...
    if (!overlaps) {
      newNotes.push({ note, startSubdiv, duration });
    }
  }
