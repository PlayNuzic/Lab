...
// App31: els enters que no són inici de cicle (ni extrems) es marquen
// .non-selectable — el drag i el clic els ignoren (fraccions complexes).
const tl = createFractionTimeline({
  ...
  decoratePulse: (el, { index, lg }) => {
    if (index === 0) el.classList.add('startpoint');
    else if (index === lg) el.classList.add('endpoint');
    else if (!isIntegerPulseSelectable(index, currentNumerator, currentDenominator, lg)) {
      el.classList.add('non-selectable');
    }
  },
  decorateSubdivision: (el, { base, lg }) => {
    const selectable = base === 0
      || isIntegerPulseSelectable(base, currentNumerator, currentDenominator, lg);
    if (!selectable) el.classList.add('non-selectable');
  },
